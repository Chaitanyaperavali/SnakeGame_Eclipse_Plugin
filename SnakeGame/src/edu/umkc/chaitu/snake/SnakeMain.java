 ...
 * @author Chaitanya Peravali
 *
 */
public class SnakeMain extends JFrame implements Runnable{
		...
		public SnakeMain() {
			...
			handler.evaluateExtensions();
			this.board = handler.getBoard();
			this.side = handler.getSide();
		...
		public void run() {
			...
			this.logicTimer = handler.getClock();//new modification
			this.isNewGame = true;
		...
		private TileType updateSnake() {
			 ...
			 * identically.
			 */
			if(head.x < 0 || head.x >= IGameBoard.COL_COUNT || head.y < 0 || head.y >= IGameBoard.ROW_COUNT) {
		...
		private void resetGame() {
			 ...
			 */
			Point head = new Point(IGameBoard.COL_COUNT / 2, IGameBoard.ROW_COUNT / 2);
		...
		private void spawnFruit() {
			 ...
			 */
			int index = random.nextInt(IGameBoard.COL_COUNT * IGameBoard.ROW_COUNT - snake.size());
